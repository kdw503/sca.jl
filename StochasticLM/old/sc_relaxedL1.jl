...
# ARGS = [":randcolX","5e-4","1","0.5","1","1","1"]
...
#=
experiments1 = [ (0.0005,200,200,10, 0.0072, 1, "pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi200_mi200_ii10_ah0.0005_nWHX2581.0_nH115930.0_objval18.19_f0.007814_rt61520.0"),
                (0.0005,200,20000,10, 0.0072, 2, "pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi200_mi20000_ii10_ah0.0005_nWHX2581.0_nH115930.0_objval18.62_f0.007815_rt35430.0"),
                (0.0005,1000,100,10, 0.0072, 3,"pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi1000_mi100_ii10_ah0.0005_nWHX2581.0_nH115750.0_objval17.28_f0.007814_rt211000.0"),
                (0.0005,400,100,10, 0.0144, 4,"pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0144_oi400_mi100_ii10_ah0.0005_nWHX2581.0_nH115910.0_objval35.41_f0.007814_rt71230.0"),
                (0.0005,400,1000,10, 0.00144, 5,"pcb_natural_randcolX_stochasticLM2_sd0.03_br0.00144_oi400_mi1000_ii10_ah0.0005_nWHX2581.0_nH115890.0_objval3.816_f0.007813_rt123300.0")
                 ]
experiments2 = [ (0.005,800,100,10, 0.0072, 1, "pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi800_mi100_ii10_ah0.005_nWHX2581.0_nH115570.0_objval171.1_f0.00781_rt136500.0"),
                (0.005,800,1000,10, 0.0072, 2, "pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi800_mi1000_ii10_ah0.005_nWHX2581.0_nH115820.0_objval179.3_f0.007814_rt119300.0"),
                (0.005,1000,100,10, 0.00144, 3,"pcb_natural_randcolX_stochasticLM2_sd0.03_br0.00144_oi1000_mi100_ii10_ah0.005_nWHX2581.0_nH115690.0_objval33.78_f0.007806_rt146000.0")
                # (0.005,400,1000,10, 0.0072, 4, "pcb_natural_randcolX_stochasticLM2_sd0.03_br0.0072_oi400_mi1000_ii10_ah0.005_nWHX2581.0_nH115860.0_objval173.4_f0.007814_rt66430.0")
                ]
...
ymin = 4915; ymax = 5000
ax = AMakie.Axis(fig[1, 1], limits = ((0,1000), (ymin, ymax)),
...
for (α, oiter,iter,iiter,br,cidx,fname) in experiments1
    ddd = load(joinpath(subworkpath,"$(fname).jld2"))
    f_xs = ddd["f_xs"]
    @show minimum(f_xs), maximum(f_xs)
    lines!(ax, f_xs, color=mtdcolors[cidx], label="α=$(α), br=$(br), maxiter=$(iter)", linestyle=nothing)
end
axislegend(ax, labelsize=fntsize1, position = :rt)
save(joinpath(subworkpath,"br_vs_penalty_α0.0005_ymax$(ymax).png"),fig, px_per_unit=2)

fig = Figure(size=resol)
ymin = 48600; ymax = 50000
ax = AMakie.Axis(fig[1, 1], limits = ((0,1000), (ymin, ymax)),
                xlabel = "iteration", ylabel = "penalty", xlabelsize=fntsize2, ylabelsize=fntsize2,
                xticklabelsize=fntsize2, yticklabelsize=fntsize2, yscale = log10)#, title = "Average Fit Value vs. Running Time")
for (α, oiter,iter,iiter,br,cidx,fname) in experiments2
    ddd = load(joinpath(subworkpath,"$(fname).jld2"))
    f_xs = ddd["f_xs"]
    @show minimum(f_xs), maximum(f_xs)
    lines!(ax, f_xs, color=mtdcolors[cidx], label="α=$(α), br=$(br), maxiter=$(iter)", linestyle=nothing)
...
axislegend(ax, labelsize=fntsize1, position = :rt)
save(joinpath(subworkpath,"br_vs_penalty_α0.005_ymax$(ymax).png"),fig, px_per_unit=2)
=#
