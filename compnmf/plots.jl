...
subworkpath = joinpath(workpath,"paper","compnmf")
...
for (prefix, initmethods, uvs) in [("compnmf",["nndsvd","rand"],["UV","LXYR"])]
    for SNR = SNRs
        rt2_min = Inf
        for initmethod = initmethods, uv = uvs
            fprex="$(prefix)$(SNR)db$(factor)f$(ncells)s$(initmethod)$(uv)"
            for iter in 1:num_experiments
                fn = joinpath(subworkpath,prefix,"$(fprex)_results$(iter).jld2")
                dd = load(fn,"data")
        ...
        stat_nnd_uv=[]; stat_nnd_lxyr=[]; stat_rnd_uv=[]; stat_rnd_lxyr=[]
        for initmethod = initmethods, uv = uvs
            afs=[]
            fprex="$(prefix)$(SNR)db$(factor)f$(ncells)s$(initmethod)$(uv)"
            for iter in 1:num_experiments
                dd = load(joinpath(subworkpath,prefix,"$(fprex)_results$(iter).jld2"))
            ...
            (initmethod == "nndsvd") && (uv == "UV") && (push!(stat_nnd_uv,means); push!(stat_nnd_uv,stds))
            (initmethod == "nndsvd") && (uv == "LXYR") && (push!(stat_nnd_lxyr,means); push!(stat_nnd_lxyr,stds))
            (initmethod == "rand") && (uv == "UV") && (push!(stat_rnd_uv,means); push!(stat_rnd_uv,stds))
            (initmethod == "rand") && (uv == "LXYR") && (push!(stat_rnd_lxyr,means); push!(stat_rnd_lxyr,stds))
        end
        fprex="$(prefix)$(SNR)db$(factor)f$(ncells)s"
        save(joinpath(subworkpath,prefix,"$(fprex)_runtime_vs_avgfits.jld2"),"rng",rng, "stat_nnd_uv", stat_nnd_uv,
                        "stat_nnd_lxyr", stat_nnd_lxyr, "stat_rnd_uv", stat_rnd_uv, "stat_rnd_lxyr", stat_rnd_lxyr)
    end
end
...
plottime = Inf
for (mtdstr, submtdstrs) in [("compnmf",["_nnd_uv", "_nnd_lxyr", "_rnd_uv", "_rnd_lxyr"])]
    @show mtdstr
...
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("compnmf","_nnd_uv","NNDSVD UV",2,nothing),
                                                                  ("compnmf","_nnd_lxyr","NNDSVD LXYR",6,:dashdot),
                                                                  ("compnmf","_rnd_uv","rand UV",4,:dash),
                                                                  ("compnmf","_rnd_lxyr","rand LXYR",5,nothing)])
