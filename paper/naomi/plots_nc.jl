...
subworkpath = joinpath(workpath,"paper","naomi/halsl2r0.5")

include(joinpath(workpath,"setup_light.jl"))
include(joinpath(workpath,"setup_plot.jl"))
using Interpolations

pavg = 5.0; num_experiments = 30; ncellss=[10,20,50]; factor=1; hals_l₁ratio=0.1
itp_time_resol = 1000
for (method, initmethods, tailstrs) in [("pcb_tsvd", ["tsvd","tsvd"], ["_sp","_sp_nn"]),
                ...
                ("hals",["nndsvd","nndsvd","nndsvd"],["_nn","_sp_nn","_sp_sm_nn"]),("compnmf",["lowrank_nndsvd"],["_nn"])]
    for ncells in ncellss
        ...
        stat_nn=[]; stat_sp=[]; stat_sp_nn=[]; stat_sp_sm_nn=[]
        for (initmethod,tailstr) in zip(initmethods,tailstrs)
            afs=[]
            for iter in 1:num_experiments
                # @show tailstr, iter
            ...
            tailstr == "_sp_sm_nn" && (push!(stat_sp_sm_nn,means); push!(stat_sp_sm_nn,stds))
        end
        save(joinpath(subworkpath,method,"$(fprex)_runtime_vs_avgfits.jld2"),"rng",rng, "stat_nn", stat_nn,
                        "stat_sp", stat_sp, "stat_sp_nn", stat_sp_nn, "stat_sp_sm_nn", stat_sp_sm_nn)
    end
end
...
for (method, submethods) in [("pcb_tsvd",["_sp", "_sp_nn"]),
                            ...
                            ("compnmf",["_nn"]),("hals",["_nn", "_sp_nn", "_sp_sm_nn"])]
    @show method
    fprex="$(method)$(pavg)mW$(factor)f$(ncells)s"
...
maxplottimes = [0.08, 0.2, 1.4]#[0.08, 0.2, 0.3, 0.4, 50.0]
...
#                                                                  ("pcb_LPF","_sp_nn","PCB LPF (α=0.005,β=5.0)",6,:dashdot),
                                                                  ...
                                                                  ("hals","_sp_nn","HALS (α=0.1, l₁ratio=1.0)",7,:dash),
                                                                  ("hals","_sp_sm_nn","HALS (α=0.1, l₁ratio=$(hals_l₁ratio))",8,:dash)]) # all
