...
for (tailstr,initmethod,β) in [("_sp",:isvd,0.)]
    ...
    save(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld"),"metadata",metadata,"data",dd)
    GC.gc()
end
# catch e
#     save(joinpath(subworkpath,"sca_error_$(iter).jld"),datadic)
#     @warn e
#     iter -= 1
# end
end
...
for tailstr in ["_sp"]
    for iter in 1:num_expriments
        dd = load(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld"),"data")
...
for tailstr in ["_sp"]
    ...
    for iter in 1:num_expriments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld"))
...
save(joinpath(subworkpath,"sca_cbcl_alpha_runtime_vs_fits.jld"),"rng",rng,
