...
dataset = :audio; SNR=0; inhibitindices=0; bias=0.1; initmethod=:lowrank_nndsvd; initpwradj=:wh_normalize
...
for (tailstr,initmethod,usennc) in [("_sp_nn",:lowrank_nndsvd,true)]
    ...
    save(joinpath(subworkpath,"admm","admm$(tailstr)_alpha_w8_results$(iter).jld"),"metadata",metadata,"data",dd)
end
...
for tailstr in ["_sp_nn"]
    for iter in 1:num_expriments
        dd = load(joinpath(subworkpath,"admm","admm$(tailstr)_alpha_w8_results$(iter).jld"),"data")
...
for tailstr in ["_sp_nn"]
    ...
    for iter in 1:num_expriments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"admm","admm$(tailstr)_alpha_w8_results$(iter).jld"))
...
save(joinpath(subworkpath,"admm_cbcl_alpha_w8_runtime_vs_fits.jld"),"rng",rng,
