...
for n_components in 2:5
    for n_neighbors in 5:5:20
        @show n_components, n_neighbors
        ...
        Legend(f[1, 2], elems, ulabel, "Cell type")
        save(joinpath(subworkpath,"UMAP_Xn_ncom$(n_components)_nnbr$(n_neighbors).png"),f,px_per_unit=2)
    end
end
...
for reduce_dims in [6, 7, 8, 9]#8:12
    for perplexity in 5:5:40
...
dd = load(joinpath(subworkpath,dataset,"aftr_nor_hclust_noc9_lkgaverage_hnothing_ne1.jld2"))
