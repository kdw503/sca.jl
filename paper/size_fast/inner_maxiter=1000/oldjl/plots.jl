...
factors = [1,5,10]; num_experiments = 50; SNR=0; ncells=15
...
z = 0.5
factors = [1,5,10]; SNR=0
for (idx,factor) = enumerate(factors)
plottime = Inf
for (mtdstr, submtdstrs) in [("lcsvd",["_sp", "_sp_nn", "_nn"]),("compnmf",["_nn"]),("hals",["_nn", "_sp_nn"])]
    @show mtdstr
...
#    @eval (($ddsym)=(load("$(mtdstr)_runtime_vs_avgfits.jld2"))) # this doens't work 'mtdstr' refer global variable
    eval(Meta.parse("$(ddstr)=load(joinpath(subworkpath,\"$(mtdstr)\",\"$(mtdstr)$(SNR)db$(factor)f$(ncells)s_runtime_vs_avgfits.jld2\"))"))
...
maxplottimes = [0.5,35.0,50.0]
ax = AMakie.Axis(fig[1, 1], limits = ((0,min(plottime,maxplottimes[idx])), (0.6,1.0)), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
...
factors = [1]; num_experiments = 50; SNR=0
for (prefix, tailstrs) in [("lcsvd", ["_sp"])]
    for factor = factors
        fprex="$(prefix)isvd$(SNR)db$(factor)"
        rt2_min = Inf
...
for (mtdstr, submtdstrs) in [("lcsvd",["_sp"]),("lcsvdisvd",["_sp"])]
    ...
    ddstr = "dd$(mtdstr)"; ddsym = Symbol(ddstr)
#    @eval (($ddsym)=(load("$(mtdstr)_runtime_vs_avgfits.jld2"))) # this doens't work 'mtdstr' refer global variable
    eval(Meta.parse("$(ddstr)=load(joinpath(subworkpath,\"$(mtdstr)\",\"$(mtdstr)$(SNR)db$(factor)_runtime_vs_avgfits.jld2\"))"))
    rng = eval(Meta.parse("$(ddsym)[\"rng\"]"))
    eval(Meta.parse("$(mtdstr)rng=$(ddsym)[\"rng\"]"))
    plottime = plottime > rng[end] ? rng[end] : plottime
    for submtdstr in submtdstrs
...
fig = Figure(resolution=(400,300))
maxplottime = 0.17
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottime#=min(maxplottimes[idx],plottime)=#), nothing), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
# for (i,(frpx, lbl)) in enumerate([("sca_sp","SMF (α=100,β=0)"),("hals_nn","HALS (α=0)"),("hals_sp_nn","HALS (α=0.1)"),
#                                  ("admm_nn","Comp. NMF (α=0)"),("admm_sp","Comp. SMF (α=10)"),
#                                  ("sca_nn","SMF (α=0,β=1000)"),("sca_sp_nn","SMF (α=100,β=1000)"),("admm_sp_nn","Comp. NMF (α=10)"),])
...
save(joinpath(subworkpath,"avgfits$(SNR)db$(factor)_compare_initmethod.png"),fig,px_per_unit=2)
