...
# ARGS = [":randcolX","0.0005","100", "100", "1e-7"]
...
@show initmethod, αh, maxiter, inner_maxiter, inner_tol
flush(stdout) 
...
(ur, nr, optim_method) = (0, 0, :lbfgs)
αh = 5e-4; inner_maxiter = 100; inner_tol = 1e-7
for initmethod in [:randcolX, :BPDN, :isvd, :randH]
    ...
    alg.show_trace = true; alg.store_trace = true; alg.store_inner_trace = false
    M1, N1t = copy(M0), copy(N0t)
    ...
    save(joinpath(subworkpath,"$(fprex)$(regstr)_it$(alg.maxiter).jld2"),"niters",niters,"f_x",f_x)
end
