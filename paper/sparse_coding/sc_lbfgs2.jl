...
for initmethod in [:randcolX, :isvd, :randH]
    ...
    fprex = "$(prefix)_$(initmethod)_$(optim_method)_W1H1_intol$(inner_tol)_initer$(inner_maxiter)"
    #fprex = "$(prefix)_BPDN"
...
#=
fname = joinpath(subworkpath,"$(prefix)_$(initmethod)_$(optim_method)_inner_maxiter_compare.jld2")
...
colors = [1,2,3,4,5,6,7,8]; maxiter = 1000; inner_tol = 1e-7; inner_maxiter = 100; optim_method = :lbfgs_admm
...
axislegend(ax1, labelsize=fntsize1, position = :rt)
axislegend(ax2, labelsize=fntsize1, position = :rt)
save(joinpath(subworkpath,"$(fprex)_alpha_vs_niters2.png"),fig1, px_per_unit=2)
save(joinpath(subworkpath,"$(fprex)_alpha_vs_f_x2.png"),fig2, px_per_unit=2)
