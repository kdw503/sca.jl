...
dataset = :neurofinder; SNR=0; inhibitindices=0; bias=0.1; initmethod=:lowrank; initpwradj=:wh_normalize
...
if subtract_bg
    Wcd, Hcd = NMF.nndsvd(X, 1, variant=:ar) # rank 1 NMF
...
for (tailstr,) in [("_sp_nn",)]
    ...
    save(joinpath(subworkpath,"hals","hals$(tailstr)_alpha_results$(iter).jld22"),"metadata",metadata,"data",dd)
end
...
for tailstr in ["_sp_nn"]
    for iter in 1:num_expriments
        dd = load(joinpath(subworkpath,"hals","hals$(tailstr)_alpha_results$(iter).jld22"),"data")
        rt2s = dd["rt2s"]
...
for tailstr in ["_sp_nn"]
    ...
    for iter in 1:num_expriments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"hals","hals$(tailstr)_alpha_results$(iter).jld22"))
...
save(joinpath(subworkpath,"hals_neurofinder_alpha_runtime_vs_fits.jld22"),"rng",rng,
