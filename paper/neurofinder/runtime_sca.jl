...
dataset = :neurofinder; SNR=0; inhibitindices=0; bias=0.1; initmethod=:isvd; initpwradj=:wh_normalize
...
# SCA
@show "SCA"
mfmethod = :SCA; penmetric = :SCA; sd_group=:whole; reg = :WH1
...
for (tailstr,initmethod,β) in [("_sp",:isvd,0.)]
    ...
    save(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld22"),"metadata",metadata,"data",dd)
    GC.gc()
end
# catch e
#     save(joinpath(subworkpath,"sca_error_$(iter).jld22"),datadic)
#     @warn e
#     iter -= 1
# end
end
...
for tailstr in ["_sp"]
    for iter in 1:num_expriments
        dd = load(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld22"),"data")
...
for tailstr in ["_sp"]
    ...
    for iter in 1:num_expriments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"sca","sca$(tailstr)_alpha_results$(iter).jld22"))
...
save(joinpath(subworkpath,"sca_neurofinder_alpha_runtime_vs_fits.jld22"),"rng",rng,
