...
# in julia REPL> ARGS = ["\"Denoise\"","[\"lcsvd_precon\",\"lcsvd_precon_LPF\",\"hals\"]", "2","-10","1","15","150",":meanT","3","3","120"]
...
if filter == :meanST 
    X = LCSVD.noisefilter(:meanS,X;filterlength=flS)
    X = LCSVD.noisefilter(:meanT,X;filterlength=flT)
elseif filter == :meanS
    X = LCSVD.noisefilter(:meanS,X;filterlength=flS)
elseif filter == :meanT
    X = LCSVD.noisefilter(:meanT,X;filterlength=flT)
