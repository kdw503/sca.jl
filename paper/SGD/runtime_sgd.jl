...
# in julia REPL> ARGS = ["\"SGD\"","[:lbfgs,:slbfgs,:sgd]", "1", "50","-10","1","15","0","30"]
...
for (prefix, maxiter) in [(:lbfgs,500),(:slbfgs,30),(:sgd,100)]
@show prefix; flush(stdout)
    #maxiter = pcb_maxiter == 0 ? Int(ceil(log(eps(eltype(X)))/log(r))) : pcb_maxiter
    useprecond = false; uselv=false; smaxiter=500; inner_maxiter = 500
...
#    try
    for (tailstr,initmethod,α,β) in [("_sp_nn",:isvd,0.005,5.0),("_nn",:nndsvd,0.,5.0), ("_sp",:isvd,0.005,0.)]#
        ...
        alg = LCSVD.LinearCombSVD(α1=α1, α2=α2, β1=β1, β2=β2, r=r, useprecond=useprecond, optim_method = prefix,
            denoisefilter=:avg, uselv=false, imgsz=imgsz, maskW=maskW, maskH=maskH, maxiter = maxiter, 
