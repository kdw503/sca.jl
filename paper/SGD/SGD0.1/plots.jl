...
subworkpath = joinpath(workpath,"paper","SGD")

include(joinpath(workpath,"setup_light.jl"))
include(joinpath(workpath,"setup_plot.jl"))
using Interpolations

SNRs = [-10, 0, 10]; num_experiments = 3; ncells=15; factor=1

for (prefix, initmethods, tailstrs) in [("lbfgs", ["isvd","isvd", "nndsvd"], ["_sp","_sp_nn", "_nn"]),
                ("slbfgs", ["isvd","isvd", "nndsvd"], ["_sp","_sp_nn", "_nn"]),
                ("sgd", ["isvd","isvd", "nndsvd"], ["_sp","_sp_nn", "_nn"])]
    for SNR = SNRs
        rt2_min = Inf
...
plottime = Inf
for (mtdstr, submtdstrs) in [("lbfgs",["_sp", "_sp_nn", "_nn"]),
                            ("slbfgs",["_sp", "_sp_nn", "_nn"]),
                            ("sgd",["_sp", "_sp_nn", "_nn"])]
    @show mtdstr
...
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lbfgs","_sp","LBFGS(α=0.005,β=0)",2,nothing),
                                                                  ("slbfgs","_sp","SLBFGS (α=0.005,β=0)",6,:dashdot),
                                                                  ("sgd","_sp","SGD (α=0.005,β=0)",4,:dash)])
...
save(joinpath(subworkpath,"avgfits$(SNR)db$(factor)f$(ncells)s_sp.png"),fig,px_per_unit=2)

fig = Figure(resolution=(400,300))
maxplottimes = [2.0,2.0,2.0]
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
# for (i,(frpx, lbl)) in enumerate([("sca_sp","LCSVD (α=100,β=0)"),("hals_nn","HALS (α=0)"),("hals_sp_nn","HALS (α=0.1)"),
#                                  ("admm_nn","Comp. NMF (α=0)"),("admm_sp","Comp. LCSVD (α=10)"),
#                                  ("sca_nn","LCSVD (α=0,β=1000)"),("sca_sp_nn","LCSVD (α=100,β=1000)"),("admm_sp_nn","Comp. NMF (α=10)"),])
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lbfgs","_nn","LBFGS(α=0,β=5.0)",2,nothing),
                                                                  ("slbfgs","_nn","SLBFGS (α=0,β=5.0)",6,:dashdot),
                                                                  ("sgd","_nn","SGD (α=0,β=5.0)",4,:dash)])
    frpx = "$(mtdstr)$(submtdstr)"
    ln = lines!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_means"))[plotrng], color=mtdcolors[clridx], label=lbl, linestyle=linestyle)
    bnd = band!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_lower"))[plotrng], eval(Symbol("$(frpx)_upper"))[plotrng], color=mtdcoloras[clridx])
    lns["$(frpx)_line"] = ln; bnds["$(frpx)_band"] = bnd;
...
axislegend(ax, labelsize=10, position = :rb) # halign = :left, valign = :top
save(joinpath(subworkpath,"avgfits$(SNR)db$(factor)f$(ncells)s_nn.png"),fig,px_per_unit=2)

fig = Figure(resolution=(400,300))
maxplottimes = [3.0,3.0,3.0]
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lbfgs","_sp_nn","LBFGS(α=0.005,β=5.0)",2,nothing),
                                                                  ("slbfgs","_sp_nn","SLBFGS (α=0.005,β=5.0)",6,:dashdot),
                                                                  ("sgd","_sp_nn","SGD (α=0.005,β=5.0)",4,:dash)])
    frpx = "$(mtdstr)$(submtdstr)"
    ln = lines!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_means"))[plotrng], color=mtdcolors[clridx], label=lbl, linestyle=linestyle)
    bnd = band!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_lower"))[plotrng], eval(Symbol("$(frpx)_upper"))[plotrng], color=mtdcoloras[clridx])
    lns["$(frpx)_line"] = ln; bnds["$(frpx)_band"] = bnd;
...
axislegend(ax, labelsize=10, position = :rb) # halign = :left, valign = :top
save(joinpath(subworkpath,"avgfits$(SNR)db$(factor)f$(ncells)s_sp_nn.png"),fig,px_per_unit=2)
