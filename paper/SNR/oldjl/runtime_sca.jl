...
subworkpath = joinpath(workpath,"paper","SNR")
...
X, imgsz, lengthT, ncs, gtncells, datadic = load_data(dataset; sigma=sigma, imgsz=imgsz, lengthT=lengthT, SNR=SNR, bias=bias, useCalciumT=true,
        inhibitindices=inhibitindices, issave=false, isload=false, gtincludebg=false, save_gtimg=true, save_maxSNR_X=true, save_X=false);
...
r=(0.3)^1
(tailstr,initmethod,α,β) = ("_sp",:isvd,0.005,0.0) # ("_sp_nn",:isvd,0.005,5.0)
tol=-1
α1=α2=α; β1=β2=β
avgfits=Float64[]; rt2s=Float64[]; inner_fxs=Float64[]
rt1 = @elapsed W0, H0, M0, N0, Wp, Hp, D = LCSVD.initlcsvd(X, ncells; initmethod=initmethod, svdmethod=:isvd)
...
W, H = copy(Wp), copy(Hp); M, N = copy(M0), copy(N0); use_σ2_cal_pen = false
...
ax = AMakie.Axis(fig[1, 1], yscale=log10, limits = ((0,0.1), (100,1000) #=(0.5,1e4)=#), xlabel = "time(sec)", ylabel = "penalty")#, title = "Average Fit Value vs. Running Time")
ax2 = AMakie.Axis(fig[1, 1], yaxisposition = :right, limits = ((0,0.1), (0.6,1.0)), ylabel = "average fit" #= yticklabelcolor = :red =# )
...
push!(lns, lines!(ax, rt2s, sparseWs, color=mtdcolors[3])); push!(lbls,"sparseWs($(σstr))")
...
save(joinpath(subworkpath,"avgfits$(SNR)db$(ncells)s_$(initmethod)_$(tailstr)_vs_penalties$(σstr).png"),fig,px_per_unit=2)


#===========================#
ncells = 15; lcsvd_maxiter = 150;
dataset = :fakecells; SNR=-10; inhibitindices=0; bias=0.1; initpwradj=:wh_normalize
filter = dataset ∈ [:neurofinder,:fakecells] ? :meanT : :none; filterstr = "_$(filter)"
datastr = dataset == :fakecells ? "_fc$(inhibitindices)_$(SNR)dB" : "_$(dataset)"
subtract_bg=false; maskth=0.25
makepositive = true; tol = -1.0;
...
mfmethod = :LCSVD; useprecond=false; uselv=false; s=10; maxiter = lcsvd_maxiter 
num_experiments = 50
usingLPFW0H0 = false; filterlength=0; afs_sp = 0; ; afs_sp_nn = 0; ; afs_nn = 0
for i in 1:num_experiments
    ...
    alg = LCSVD.LinearCombSVD(α1=α1, α2=α2, β1=β1, β2=β2, σ0=σ0, r=r, useprecond=false, uselv=false, maskW=maskW, maskH=maskH,
        maxiter = maxiter, store_trace = true, store_inner_trace = true, show_trace = false, allow_f_increases = true,
        f_abstol=tol, f_reltol=tol, f_inctol=1e2, x_abstol=tol, successive_f_converge=0, W0=W0, H0=H0)
    W0lpf = usingLPFW0H0 ? noisefilter(:meanS,W0,imgsz=imgsz,filterlength=filterlength) : copy(W0)
    H0lpf = usingLPFW0H0 ? noisefilter(:meanT,filterlength=filterlength,H0) : copy(H0) 
    # W, H = copy(Wp), copy(Hp); M, N = copy(M0), copy(N0)
    # rst = LCSVD.solve!(alg, X, W0lpf, H0lpf, D, M, N, W, H; gtW=gtW, gtH=gtH);
    alg.store_trace = false; alg.store_inner_trace = false
    W, H = copy(Wp), copy(Hp); M, N = copy(M0), copy(N0)
    rt2 = @elapsed rst0 = LCSVD.solve!(alg, X, W0lpf, H0lpf, D, M, N, W, H; gtW=gtW, gtH=gtH);
    W, H = W0*M, N*H0
    avgfit, ml, merrval, rerrs = SCA.matchedfitval(gtW, gtH, W, H; clamp=false)
    normalizeW!(W,H); W3,H3 = sortWHslices(W,H)
    fprex = "$(prefix)$(SNR)db$(ncells)s$(initmethod)"
    fname = joinpath(subworkpath,"$(fprex)_a$(α)_b$(β)_af$(avgfit)_fl$filterlength)")
    #imsave_data(dataset,fname,W3,H3,imgsz,100; saveH=false)
    TestData.imsave_data_gt(dataset,fname*"_gt", W3,H3,gtW,gtH,imgsz,100; saveH=false, verbose=false)
    afs_sp += avgfit

    (tailstr,initmethod,α,β) = ("_sp_nn",:isvd,0.005,5.0) # ("_sp_nn",:isvd,0.005,5.0)
    α1=α2=α; β1=β2=β
    alg = LCSVD.LinearCombSVD(α1=α1, α2=α2, β1=β1, β2=β2, σ0=σ0, r=r, useprecond=false, uselv=false, maskW=maskW, maskH=maskH,
        maxiter = maxiter, store_trace = true, store_inner_trace = true, show_trace = false, allow_f_increases = true,
        f_abstol=tol, f_reltol=tol, f_inctol=1e2, x_abstol=tol, successive_f_converge=0, W0=W0, H0=H0)
    ...
    (tailstr,initmethod,α,β) = ("_nn",:nndsvd,0.0,0.0) # ("_sp_nn",:isvd,0.005,5.0)
    α1=α2=α; β1=β2=β
    rt1 = @elapsed W0, H0, M0, N0, Wp, Hp, D = LCSVD.initlcsvd(X, ncells; initmethod=initmethod, svdmethod=:isvd)
    σ0=s*std(W0) #=10*std(W0)=#
    r=(0.3)^1 #0.3 # decaying rate for relaxed L1, if this is too small result is very sensitive for setting α
...
afs_sp /= num_experiments
afs_nn /= num_experiments
afs_sp_nn /= num_experiments
...
(tailstr,initmethod,α,β) = ("_sp",:isvd,0.005,0.)
@show tailstr
dd = Dict(); tol=-1
α1=α2=α; β1=β2=β
avgfits=Float64[]; rt2s=Float64[]; inner_fxs=Float64[]
...
alg = LCSVD.LinearCombSVD(α1=α1, α2=α2, β1=β1, β2=β2, σ0=σ0, r=r, useprecond=false, usedenoiseW0H0=usedenoiseW0H0, 
    denoisefilter=:mean, uselv=false, imgsz=imgsz, maskW=maskW, maskH=maskH, maxiter = maxiter, store_trace = false,
    store_inner_trace = false, show_trace = false, allow_f_increases = true, f_abstol=tol, f_reltol=tol, f_inctol=1e2,
    x_abstol=tol, x_reltol=tol, successive_f_converge=0)
M, N = copy(M0), copy(N0)
rst1 = LCSVD.solve!(alg, X, W0, H0, D, M, N; gtW=gtW, gtH=gtH);
W1, H1 = rst1.W, rst1.H
avgfit1, ml, merrval, rerrs = SCA.matchedfitval(gtW, gtH, W1, H1; clamp=false)

alg.denoisefilter = :med
M, N = copy(M0), copy(N0)
rst2 = LCSVD.solve!(alg, X, W0, H0, D, M, N; gtW=gtW, gtH=gtH);
W2, H2 = rst2.W, rst2.H
avgfit2, ml, merrval, rerrs = SCA.matchedfitval(gtW, gtH, W2, H2; clamp=false)

alg.usedenoiseW0H0 = false
M, N = copy(M0), copy(N0)
rt2 = @elapsed rst0 = LCSVD.solve!(alg, X, W0, H0, D, M, N; gtW=gtW, gtH=gtH);
W, H = rst0.W, rst0.H
avgfit0, ml, merrval, rerrs = SCA.matchedfitval(gtW, gtH, W, H; clamp=false)
if  true #avgfit1 < avgfit0
    @show "not improved"
    fprex = "$(prefix)$(SNR)db$(ncells)s_$(iter)"
    W0lpf=noisefilter(:meanS,W0,imgsz=imgsz,filterlength=3)
    H0lpf = noisefilter(:meanT,filterlength=5,H0)
    W0med=noisefilter(:medS,W0,imgsz=imgsz,filterlength=3)
    H0med = noisefilter(:medT,filterlength=5,H0)
    imsaveW(joinpath(subworkpath,"$(fprex)_SVD_LPF_W0.png"), W0lpf[:,1:10],imgsz)
    plotH_data(joinpath(subworkpath,"$(fprex)_SVD_LPF)"), H0lpf[1:10,:])
    imsaveW(joinpath(subworkpath,"$(fprex)_SVD_MED_W0.png"), W0med[:,1:10],imgsz)
    plotH_data(joinpath(subworkpath,"$(fprex)_SVD_MED)"), H0med[1:10,:])
    imsaveW(joinpath(subworkpath,"$(fprex)_SVD_W0.png"), W0[:,1:10],imgsz)
    plotH_data(joinpath(subworkpath,"$(fprex)_SVD)"), H0[1:10,:])

    normalizeW!(W1,H1); W3,H3 = sortWHslices(W1,H1)
    fname = joinpath(subworkpath,"$(fprex)_a$(α)_b$(β)_af$(avgfit1)")
    TestData.imsave_data_gt(dataset,fname*"_LPF_gt", W3,H3,gtW,gtH,imgsz,100; saveH=false, verbose=false)
    normalizeW!(W2,H2); W3,H3 = sortWHslices(W2,H2)
    fname = joinpath(subworkpath,"$(fprex)_a$(α)_b$(β)_af$(avgfit2)")
    TestData.imsave_data_gt(dataset,fname*"_MED_gt", W3,H3,gtW,gtH,imgsz,100; saveH=false, verbose=false)
    normalizeW!(W,H); W3,H3 = sortWHslices(W,H)
    fname = joinpath(subworkpath,"$(fprex)_a$(α)_b$(β)_af$(avgfit0)")
    TestData.imsave_data_gt(dataset,fname*"_gt", W3,H3,gtW,gtH,imgsz,100; saveH=false, verbose=false)
end
