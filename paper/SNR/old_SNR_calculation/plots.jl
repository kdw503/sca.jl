...
subworkpath = joinpath(workpath,"paper","SNR2")

include(joinpath(workpath,"setup_light.jl"))
include(joinpath(workpath,"setup_plot.jl"))
using Interpolations

SNRs = [0,10,20,30,40]; num_experiments = 20; ncells=15; factor=1
...
maxplottimes = [0.20,0.20,0.20,0.20,0.20]
...
fig = Figure(size=(400,300))
maxplottimes = [0.05,0.05,0.05,0.05,0.05]
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
...
fig = Figure(size=(400,300))
maxplottimes = [0.3,0.3,0.3,0.3,0.3]
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
