...
SNRs = [-10,0,10]; num_experiments = 50; ncells=15; factor=1
...
for (prefix, initmethods, tailstrs) in [("lcsvd", ["isvd","isvd", "nndsvd"], ["_sp","_sp_nn", "_nn"]),
    ...
    for SNR = SNRs
        ...
        islcsvd = prefix ∈ ["lcsvd","lcsvd_LPF","lcsvd_precon","lcsvd_precon_LPF"]
        ...
        for (initmethod,tailstr) in zip(initmethods,tailstrs)
            ...
            for iter in 1:num_experiments
                ...
                rt2s = dd["data"]["rt2s"]; pen = dd["data"][islcsvd ? "inner_fxs" : "f_xs"]
                lr = length(rt2s); la = length(pen)
                lr != la && (l=min(lr,la); rt2s=rt2s[1:l]; pen=pen[1:l])
                nodes = (rt2s,)
                itp = Interpolations.interpolate(nodes, pen, Gridded(Linear()))
                push!(afs,itp(rng))
            end
            pen = hcat(afs...)
            means = dropdims(mean(pen,dims=2),dims=2)
            stds = dropdims(std(pen,dims=2),dims=2)
        ...
        save(joinpath(subworkpath,prefix,"$(fprex)_runtime_vs_pen.jld2"),"rng",rng, "stat_nn", stat_nn,
                        "stat_sp", stat_sp, "stat_sp_nn", stat_sp_nn)
    end
end

tmppath = ""
z = 0.5; ylimits=nothing; tol=1e-6
...
#    @eval (($ddsym)=(load("$(mtdstr)_runtime_vs_pen.jld2"))) # this doens't work 'mtdstr' refer global variable
    eval(Meta.parse("$(ddstr)=load(joinpath(subworkpath,tmppath,\"$(mtdstr)\",\"$(mtdstr)$(SNR)db$(factor)f$(ncells)s_runtime_vs_pen.jld2\"))"))
...
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)",
    yscale=log10, ylabel = "penalty")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd","_sp","no precond",2,nothing),
    ...
    eval(Symbol("$(frpx)_lower"))[eval(Symbol("$(frpx)_lower")).<tol] .= tol
...
idx == 3 && axislegend(ax, position = :rt) # halign = :left, valign = :top
save(joinpath(subworkpath,"pen_sp_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd","_nn","no precond",2,nothing),
                                                                ("lcsvd_LPF","_nn","noprecond LPF",6,nothing),
...
idx == 3 && axislegend(ax, position = :rt) # halign = :left, valign = :top
save(joinpath(subworkpath,"pen_nn_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
save(joinpath(subworkpath,"pen_sp_nn_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd","_sp","LCSVD (α=0.005,β=0)",2,nothing),
                                                                  ("lcsvd","_nn","LCSVD (α=0,β=5.0)",6,:dashdot),
...
idx == 3 && axislegend(ax, position = :rt) # halign = :left, valign = :top
save(joinpath(subworkpath,"pen$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
