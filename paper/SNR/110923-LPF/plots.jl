...
subworkpath = joinpath(workpath,"paper","SNR","110923-LPF")
...
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd_precon","_sp","precond",4,nothing),
...
save(joinpath(subworkpath,"compare_LPF","avgfits_sp_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd_precon","_nn","precond",4,nothing),
...
axislegend(ax, position = :rb) # halign = :left, valign = :top
save(joinpath(subworkpath,"compare_LPF","avgfits_nn_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
ax = AMakie.Axis(fig[1, 1], limits = ((0,maxplottimes[idx]#=min(maxplottimes[idx],plottime)=#), ylimits), xlabel = "time(sec)", ylabel = "average fit")#, title = "Average Fit Value vs. Running Time")
lns = Dict(); bnds=Dict()
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("lcsvd","_sp_nn","no precond",4,nothing),
                                                                ("lcsvd_LPF","_sp_nn","no precond LPF",5,nothing)]) # all
    # eval(print("$(frpx)_means"))
    frpx = "$(mtdstr)$(submtdstr)"
    ln = lines!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_means"))[plotrng], color=mtdcolors[clridx], label=lbl, linestyle=linestyle)
    bnd = band!(ax, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)_lower"))[plotrng], eval(Symbol("$(frpx)_upper"))[plotrng], color=mtdcoloras[clridx])
...
axislegend(ax, position = :rb) # halign = :left, valign = :top
save(joinpath(subworkpath,"compare_LPF","avgfits_sp_nn_$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
...
save(joinpath(subworkpath,"compare_LPF","avgfits$(SNR)db$(factor)f$(ncells)s_all.png"),fig,px_per_unit=2)
