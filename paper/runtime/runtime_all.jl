...
for (tailstr,initmethod,α,β) in [("_sp",:isvd,100.,0.)]
    ...
    rt2 = @elapsed W1, H1, objvals, laps, trace, _ = scasolve!(X, W0, H0, D, Mw, Mh, Wp, Hp; gtW=gtW, gtH=gtH,
...
# ADMM
@show "ADMM"
mfmethod = :ADMM; penmetric = :SCA; sd_group=:whole; regSpar = :WH1; α = 10; β = 0; usennc=true
...
for (tailstr,initmethod,α,usennc) in [("_nn",:lowrank_nndsvd,0.,true)]
    @show tailstr; flush(stdout)
    rt1 = @elapsed W0, H0, Mw0, Mh0, Wp, Hp, D = initsemisca(X, ncells, initmethod=initmethod,poweradjust=initpwradj)
    σ0=s*std(W0) #=10*std(W0)=#
    avgfits=Float64[]; rt2s=Float64[]; inner_fxs=Float64[]
...
α = 0.5; ridge_alpha=0.01; maxiter=500; tol=1e-7
rtspca = @elapsed resultspca = fit_transform!(SparsePCA(n_components=ncells,alpha=α,ridge_alpha=ridge_alpha,max_iter=maxiter,tol=tol,verbose=false),X) 
W1 = copy(resultspca); H1 = W1\X
avgfit, ml, merrval, rerrs = SCA.matchedfitval(gtW, gtH, W1, H1; clamp=false)
normalizeW!(W1,H1); W3,H3 = sortWHslices(W1,H1)
makepositive && flip2makepos!(W3,H3)
fprex = "$(mfmethod)$(datastr)$(filterstr)"
fname = joinpath(subworkpath,"$(fprex)_a$(α)_af$(avgfit)_it$(maxiter)_rt$(rtspca)")
#imsave_data(dataset,fname,W3,H3,imgsz,100; saveH=false)
imsave_data_gt(dataset,fname*"_gt", W3,H3,gtW,gtH,imgsz,100; saveH=true)
