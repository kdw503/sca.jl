...
for (tailstr,initmethod,α,usennc) in [("_nn",:lowrank_nndsvd,0.,true)]#, ("_sp",:lowrank,10.,false), ("_sp_nn",:lowrank_nndsvd,10.,true)]
    ...
    save(joinpath(subworkpath,"admm","admm$(tailstr)_results$(iter).jld2"),"metadata",metadata,"data",dd)
end
...
for tailstr in ["_nn"]#,"_sp","_sp_nn"]
    for iter in 1:num_expriments
        dd = load(joinpath(subworkpath,"admm","admm$(tailstr)_results$(iter).jld2"),"data")
...
for tailstr in ["_nn"]#,"_sp","_sp_nn"]
    ...
    for iter in 1:num_expriments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"admm","admm$(tailstr)_results$(iter).jld2"))
...
save(joinpath(subworkpath,"admm_runtime_vs_avgfits.jld2"),"rng",rng, "stat_nn", stat_nn, "stat_sp", stat_sp, "stat_sp_nn", stat_sp_nn)
