...
for (i,(mtdstr,submtdstr,lbl)) in enumerate([("sca","_sp","SMF (α∈[1,1000])"),("admm","_nn","Comp. NMF)"),("hals","_sp_nn","HALS (α∈[0,5])")])
    ...
    ln1 = lines!(ax11_1, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)1_means"))[plotrng], color=mtdcolors[i+1], label=lbl)
    bnd1 = band!(ax11_1, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)1_lower"))[plotrng], eval(Symbol("$(frpx)1_upper"))[plotrng], color=mtdcoloras[i+1])
    ln2 = lines!(ax11_2, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)2_means"))[plotrng], color=mtdcolors[i+1], label=lbl, linestyle = :dash, linewidth = 2)
    bnd2 = band!(ax11_2, eval(Symbol("$(mtdstr)rng"))[plotrng], eval(Symbol("$(frpx)2_lower"))[plotrng], eval(Symbol("$(frpx)2_upper"))[plotrng], color=mtdcoloras[i+1])
    ln1s["$(frpx)_line1"] = ln1; bnd1s["$(frpx)_band1"] = bnd1;
    ln2s["$(frpx)_line2"] = ln2; bnd2s["$(frpx)_band2"] = bnd2;
    lbls["$(frpx)_label"] = lbl
end

fig[1,2] = Legend(fig,[collect(values(ln1s)),collect(values(ln2s))],[collect(values(lbls)),collect(values(lbls))],["Fit", "Sparsity |W|"])
# axislegend(ax1, position = :rt) # halign = :left, valign = :top
# axislegend(ax2, position = :rb) # halign = :left, valign = :top
...
ddadmmw1 = load(joinpath(subworkpath,"admm_cbcl_alpha_w1_runtime_vs_fits.jld2")); rng = ddadmmw1["rng"]
...
ddadmmw8 = load(joinpath(subworkpath,"admm_cbcl_alpha_w8_runtime_vs_fits.jld2")); rng = ddadmmw8["rng"]
