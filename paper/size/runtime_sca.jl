...
if true
    num_experiments = 20
    # sca_maxiter = 50; sca_inner_maxiter = 50; sca_ls_maxiter = 100
    # admm_maxiter = 400; admm_inner_maxiter = 0; admm_ls_maxiter = 0
    # hals_maxiter = 50; maskth=0.25 # for factor == 10
    sca_maxiter = 150; sca_inner_maxiter = 50; sca_ls_maxiter = 100
    admm_maxiter = 800; admm_inner_maxiter = 0; admm_ls_maxiter = 0
    hals_maxiter = 120; maskth=0.25 # for factor == 1
else
    num_experiments = 1
    sca_maxiter = 1; sca_inner_maxiter = 50; sca_ls_maxiter = 2
...
X, imgsz, lengthT, ncells, gtncells, datadic = load_data(dataset; sigma=sigma, imgsz=imgsz, lengthT=lengthT, SNR=SNR, bias=bias, useCalciumT=true,
...
try
for (tailstr,initmethod,α,β) in [("_sp_nn",:nndsvd,0.005,5.0),("_nn",:nndsvd,0.,5.0), ("_sp",:nndsvd,0.005,0.)]# 
...
for (tailstr,α) in [("_nn",0.),("_sp_nn",0.1)]#
    ...
    result = NMF.solve!(NMF.CoordinateDescent{Float64}(maxiter=maxiter, α=α, l₁ratio=1,
                    tol=tol, verbose=true), X, W1, H1; W0=W0, H0=H0, d=diag(D), gtW=gtW, gtH=gtH,
