...
filter = dataset ∈ [:neurofinder,:fakecells] ? :meanT : :none; filterstr = "_$(filter)"
datastr = dataset == :fakecells ? "_fc$(inhibitindices)_$(SNR)dB" : "_$(dataset)"
subtract_bg=false; maskth=0.25; makepositive = true; tol = -1.0;
...
uselv=false; s=10; maxiter = lcsvd_maxiter ; useprecond=false; usedenoiseW0H0=false
...
for (tailstr,initmethod,α,β) in [("_sp_nn",:isvd,0.005,5.0),("_nn",:nndsvd,0.,5.0), ("_sp",:isvd,0.005,0.)]#
    ...
    fname = joinpath(subworkpath,prefix,"$(fprex)$(precondstr)$(useLPFstr)_a$(α)_b$(β)_af$(avgfit)_it$(rst0.niters)_rt$(rt2)")
