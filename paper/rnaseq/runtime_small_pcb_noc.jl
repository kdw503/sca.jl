...
# sizestep, pcb_maxiter, pcb_inner_maxiter, tol, per_component, bothreg, nocrng = 20, 0, 1000, 1e-6, false, true, [300,400]
...
# LCSVD
prefix = "pcb"
exprstr = bothreg ? "$(prefix) noc both" : "$(prefix) noc each" 
@show exprstr, sizestep, nocrng; flush(stdout)
...
#nocrng = [300,400,500,600,700,800,900,1000]

for (iter, noc) in enumerate(nocrng)
@show iter, noc; flush(stdout)
nac = 0; nc = noc+nac
...
jldfprex = "$(file_name)$(Xtpstr)_ss$(sizestep)_pcb_nocrng_awbh"
fname = joinpath(subworkpath,"$(jldfprex).jld2")
...
save(joinpath(subworkpath,"fit_and_sparsity_nocrng_aw$(α)_bh$(β).png"),fig)
