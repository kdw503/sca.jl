...
# sizestep, pcb_maxiter, pcb_inner_maxiter, tol, per_component, βrng = 20, 1, 1, 1e-6, false, [0.1,3.0]
...
# LCSVD
prefix = "pcb"
exprstr = bothreg ? "$(prefix) beta both" : "$(prefix) beta each" 
@show exprstr, sizestep, βrng; flush(stdout)
...
for (iter, β) in enumerate(βrng)
    @show iter, β; flush(stdout)
    sbc_maxiter = 100
...
jldfprex = "$(file_name)$(Xtpstr)_ss$(sizestep)_pcb_noc$(noc)_βhrng"
fname = joinpath(subworkpath,"$(jldfprex).jld2")
save(fname, "βrng", βrng,"fitvals",fitvals,"sws",sws ,"per_component", per_component, "optim_method", optim_method, "α", α, "r",r,"tol",tol)
dd = load(fname)
βrng = dd["βrng"]; fitvals = dd["fitvals"]; sws = dd["sws"]

labels = ["fitval","norm(W,1)"]
fig = Figure(resolution = (700,400))
ax1 = AMakie.Axis(fig[1, 1], limits=(nothing, (0.,1.1) ), xlabel = "βh", ylabel = "fit", title = "Fit Value vs. βh")
