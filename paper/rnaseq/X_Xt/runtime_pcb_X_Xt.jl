...
# \"WMB-10Xv2-HY\" 0 500 0 0.005 0.005 5.0 5.0 0
...
method = "pcb"
useprecond=false; uselv=false; tol=1e-6; pcb_maxiter = 1
r=0.3; maxiter = pcb_maxiter == 0 ? Int(ceil(log(eps(eltype(X)))/log(r))) : pcb_maxiter
inner_tol = 1e-6; inner_maxiter = 1#Int(ceil(2.5*ncells+350))

# PCB(ISVD(X))
@show "PCB(ISVD(X))"

initmethod = :isvd; initmtdstr="init$(initmethod)"
fname = joinpath(subworkpath,"$(file_name)t_$(initmtdstr)_noc$(noc)_nac$(nac)_X.jld2")
if isfile(fname)
    ...
    fn = joinpath(subworkpath,"$(file_name)t_isvd_nc$(nc).jld2")
    if isfile(fn)
        @show "Reading isvd..."
        dd = load(fn)
        ...
        rt0 = @elapsed ((U, s) = isvd(Xt, nc); Vt = Array(Diagonal(s.^-1)*(U'*Xt))) # Vt = Array((pinv(U*Diagonal(s))*X))
...
alg = LCSVD.LinearCombSVD(α1=α2, α2=α1, β1=β2, β2=β1, r=r, useprecond=useprecond,
...
rt2 = @elapsed rst0 = LCSVD.solve!(alg, Xt, W0, H0t, D, M, Nt);
...
#fitval = LCSVD.fitd(Xt,W*H)
jldfprex = "$(file_name)t_$(initmtdstr)_$(method)_noc$(noc)_nac$(nac)_aw$(α2)_ah$(α1)_bw$(β2)_bh$(β1)_tol$(tol)_it$(iter)"
fname = joinpath(subworkpath,"$(jldfprex).jld2")
save(fname, "W",W,"H",H,"M",M,"N",Nt',"rt1",rt1,"rt2",rt2,"iter",iter,"rst",rst0)
...
W0, H0t, M0, N0t =  Array(H0t'), Array(W0'), Array(N0t'), Array(M0')
