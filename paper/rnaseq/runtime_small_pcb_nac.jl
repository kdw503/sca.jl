...
# sizestep, pcb_maxiter, pcb_inner_maxiter, tol, per_component, bothreg, nacrng = 20, 0, 1000, 1e-6, false, true, [300,400]
...
# LCSVD
prefix = "pcb"
exprstr = bothreg ? "$(prefix) nac both" : "$(prefix) nac each" 
@show exprstr, sizestep, nacrng; flush(stdout)
...
for (iter, nac) in enumerate(nacrng)
@show iter, nac; flush(stdout)
nc = noc+nac
...
jldfprex = "$(file_name)$(Xtpstr)_ss$(sizestep)_pcb_noc$(noc)_nac$(nac)_aw$(α)_bh$(β)_it$(rst0.niters)"
fname = joinpath(subworkpath,"$(jldfprex).jld2")
save(fname, "W",W,"H",H,"M",M,"N",Nt',"rt1",rt1,"rt2",rt2,"iter",rst0.niters,"fitval",fitval,"nsw",nsw)
push!(fitvals,fitval)
push!(nsws,nsw)
...
jldfprex = "$(file_name)$(Xtpstr)_ss$(sizestep)_pcb_nacrng_awbh"
fname = joinpath(subworkpath,"$(jldfprex).jld2")
save(fname, "nacrng", nacrng,"fitvals",fitvals,"nsws",nsws ,"per_component", per_component,
    "optim_method", optim_method, "α", α, "β",β,"noc",noc,"r",r,"tol",tol)

labels = ["fitval","norm(W,1)/noc"]
fig = Figure(resolution = (500,300))
ax1 = AMakie.Axis(fig[1, 1], limits=((300,1000), (0.95,1.05)), xlabel = "nac", ylabel = "fit", title = "Fit Value vs. nac")
ax2 = AMakie.Axis(fig[1, 1], limits=((300,1000), (1000.,3000)), yaxisposition = :right, ylabel = "Sparseness of W" #= yticklabelcolor = :red =# )
