...
if true
    ...
    admm_maxiter = 3000; admm_inner_maxiter = 0; admm_ls_maxiter = 0
    hals_maxiter = 200
    αrng = 0:0.2:50
else
    num_experiments = 2
...
for iter in 19:num_experiments
...
for (tailstr,initmethod,usennc) in [("_sp",:lowrank,false), ("_sp_nn",:lowrank_nndsvd,true)]
    ...
    if true#iter == num_experiments
        ...
        metadata["αrng"] = αrng; metadata["beta"] = 0; metadata["optimmethod"] = optimmethod; metadata["initmethod"] = initmethod
    end
    save(joinpath(workpath,"paper","alpha","admm$(tailstr)_alpha_results$(iter).jld2"),"metadata",metadata,"avgfits",avgfits)
end
end
...
num_expriments=3
dd = load(joinpath(workpath,"paper","alpha","admm_sp_nn_alpha_results1.jld2"))
...
for tailstr in ["_sp","_sp_nn"]
    ...
    for iter in 1:num_expriments
        dd = load(joinpath(workpath,"paper","alpha","admm$(tailstr)_alpha_results$(iter).jld2"))
        avgfits = dd["avgfits"]
        nodes = (αrng,)
...
save(joinpath(workpath,"paper","alpha","admm_alpha_vs_avgfits.jld2"),"rng", rng, "stat_sp", stat_sp, "stat_sp_nn", stat_sp_nn)
=#