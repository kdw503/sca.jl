...
for (tailstr,initmethod,α,β) in [("_sp",:isvd,100.,0.), ("_sp_nn",:nndsvd,100.,1000.)] #("_nn",:nndsvd,0.,1000.), 
    ...
    save(joinpath(subworkpath,"sca","sca$(ncells)$(tailstr)_results$(iter).jld2"),"metadata",metadata,"data",dd)
    GC.gc()
end
catch e
    save(joinpath(subworkpath,"sca$(ncells)_error_$(iter).jld2"),datadic)
    @warn e
    iter -= 1
end
end # for ncells
...
for tailstr in ["_sp","_sp_nn"]
    for iter in 1:num_experiments
        dd = load(joinpath(subworkpath,"sca","sca$(ncells)$(tailstr)_results$(iter).jld2"),"data")
...
for tailstr in ["_sp","_sp_nn"]
    ...
    for iter in 1:num_experiments
        @show tailstr, iter
        dd = load(joinpath(subworkpath,"sca","sca$(ncells)$(tailstr)_results$(iter).jld2"))
...
save(joinpath(subworkpath,"sca$(ncells)_runtime_vs_avgfits.jld2"),"rng",rng, "stat_nn", stat_nn, "stat_sp", stat_sp, "stat_sp_nn", stat_sp_nn)
end
...
for (tailstr,α) in [("_nn",0.),("_sp_nn",0.1)]
    ...
    save(joinpath(subworkpath,"hals","hals$(ncells)$(tailstr)_results$(iter).jld2"),"metadata",metadata,"data",dd)
end
end # for ncells
end # for iter
...
for tailstr in ["_nn","_sp_nn"]
    for iter in 1:num_experiments
        dd = load(joinpath(subworkpath,"hals","hals$(ncells)$(tailstr)_results$(iter).jld2"),"data")
        rt2s = dd["rt2s"]
        rt2_min = min(rt2_min,rt2s[end])
    end
end
...
for tailstr in ["_nn","_sp_nn"]
    afs=[]
    for iter in 1:num_experiments
        dd = load(joinpath(subworkpath,"hals","hals$(ncells)$(tailstr)_results$(iter).jld2"))
        rt2s = dd["data"]["rt2s"]; avgfits = dd["data"]["avgfits"]
        nodes = (rt2s,)
        itp = Interpolations.interpolate(nodes, avgfits, Gridded(Linear()))
        push!(afs,itp(rng))
...
save(joinpath(subworkpath,"hals$(ncells)_runtime_vs_avgfits.jld2"),"rng",rng, "stat_nn", stat_nn, "stat_sp_nn", stat_sp_nn)
end
