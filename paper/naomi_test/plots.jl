...
subworkpath = joinpath(workpath,"paper","naomi_test")

include(joinpath(workpath,"setup_light.jl"))
include(joinpath(workpath,"setup_plot.jl"))
using Interpolations

SNR = 0; num_experiments = 8; ncellss=[15]; factor=1
itp_time_resol = 1000
for (prefix, initmethods, tailstrs) in [("pcb_tsvd", ["tsvd","tsvd"], ["_sp","_sp_nn"]),
...
plottime = Inf
for (mtdstr, submtdstrs) in [("pcb_tsvd",["_sp", "_sp_nn"]),
...
for (i,(mtdstr, submtdstr, lbl, clridx, linestyle)) in enumerate([("pcb_tsvd","_sp","PCB (α=0.005,β=0)",2,nothing),
