...
X, imgsz, lengthT, ncells, gtncells, datadic = load_data(dataset; SNR=SNR, bias=bias, useCalciumT=true,
        inhibitidx=inhibitidx, issave=true, isload=true, gtincludebg=false, save_gtimg=true, save_maxSNR_X=false, save_X=false);
...
for (tailstr,initmethod,α,β) in [("_nn",:nndsvd,0.,1000.), ("_sp",:isvd,100.,0.), ("_sp_nn",:nndsvd,100.,1000.)]
    ...
    fxs = getdata(trs,:f_x); fxss = getdata(trs,:fxs)
...
#            avgfit = fitdold(X,W3*H3)
    ...
    axi.plot(avgfits); axt.plot(rt2s,avgfits)

    lines!(ax, x, y, color=:blue)
    lines!(ax, x, y, color=:blue)

    #plot(rt2s[1:25:length(rt2s)],avgfits[1:25:length(avgfits)])
end
...
for withlegend = [false, true]
    ...
    figi.savefig("$(fnprfx)_iter_$(withlegend).png")
    figt.savefig("$(fnprfx)_time_$(withlegend).png")
end
close("all")
